...
def create_layers(template, data_path, rasterfiles):
    ...
    for item in rasterfiles:
        ...
        sub_dir = item[0] or ""
        source_srs = rasterutils.calculate_source_projection(os.path.join(data_path, sub_dir, file_name))
        dictionary = create_layer_dic(name, group, source_srs, os.path.join(sub_dir, file_name))
        layers.append(template.substitute(dictionary))
    return layers

def generate_tif_config(data_path, map_path, debug):
    main_template_path = os.path.join(resource_dir,"templates","geotif_templates","geotif_main.map")
    layer_template_path = os.path.join(resource_dir,"templates","geotif_templates","geotif_layer.map")
    ...
    if layers:
        extent = rasterutils.calculate_total_extent(data_path, rasterfiles, output_srs)
        dictionary2 = create_main_dic(data_path, map_path, extent, output_srs, layers, debug)
