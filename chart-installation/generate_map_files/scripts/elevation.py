...
def create_layers(template, files, data_path):
    ...
    for f in files:
        ...
        dictionary = create_layer_dic(file_name, group_name, os.path.relpath(file_path, data_path), source_srs or output_srs)
...
#       Handle files in subfolder
        if folder:
            ...
            with open(filename, 'a') as vrttemp:
                ...
                processes.append(Process(target=generate_virtual_and_overview, args=(elev_gen_path, hill_gen_path, folder, folder, filename)))
#       Handle files directly under the data dir
            ...
            for g in list(group):
                ...
                processes.append(Process(target=generate_virtual_and_overview, args=(elev_gen_path, hill_gen_path, None, os.path.splitext(g[1])[0], filename)))
...
def create_map_file(data_path, map_path, mapfile_name, file_suffix, main_template, layer_template, debug):
    ...
    if layers:
        total_extent = rasterutils.calculate_total_extent(data_path, files, output_srs)
        dictionary = create_main_dic(data_path, map_path, total_extent, output_srs, layers, debug)
...
def generate_elevation_config(data_path, map_path, debug):
    ...
    templates_dir = os.path.join(resource_dir,"templates","elevation_templates")
    ...
    hillshade_main_template_path = os.path.join(templates_dir,"hillshade_main.map")
    hillshade_layer_template_path = os.path.join(templates_dir,"hillshade_layer.map")
    ...
    hillshade_main_template = Template(open(hillshade_main_template_path, 'r').read())
    hillshade_layer_template = Template(open(hillshade_layer_template_path, 'r').read())

#   Do data preparation 
    prepare_data(data_path, elev_gen_path, hill_gen_path)

#   Create elevation map file
    create_map_file(elev_gen_path, map_path, 'Elevation.map', ['.tif'], main_template, layer_template, debug)
