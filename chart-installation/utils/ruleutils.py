...
def create_color_rules(resource_dir, path, default_color_table):
    ...
    path_to_selection = os.path.join(resource_dir, "chartsymbols", choices[selection])
    clear_folder(force_sub_dir(path, "color_tables"))
    bashCmd = ' '.join(["./convert_colors_xml2csv.sh","input=" + path_to_selection, " target=" + path])
    subprocess.call(bashCmd, shell=True)

def create_layer_rules(resource_dir, config_path):
    print "Overwrite color tables in " + config_path
    copy_and_replace(os.path.join(resource_dir, "layer_rules"), force_sub_dir(config_path, "layer_rules"))
