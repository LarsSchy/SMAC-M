...
def parse_arguments():
    parser = argparse.ArgumentParser(
        prog="S57_to_Shape", description="This program converts a S57 source to Shape that is parseable by the map generating scripts")
    parser.add_argument("src_dir", nargs=1, help="Directory of your S57 files")
    parser.add_argument("out_dir", nargs=1,
                        help="Directory of your Shape output")
    parser.add_argument("-progress_addr", default='',
                        help="Where to send progress information")
    parser.add_argument("-d", "--debug", action="store_true",
                        help="Enable debug for this operation")
    parser.add_argument("-log", choices=['no', 'tmp', 'stdout'], default='no',
                        help="Choose if you want 'no' logging, logging to /tmp or logging to standard out")
...
def main():
    ...
    output_path = dirutils.force_sub_dir(
        os.path.abspath(args.out_dir[0]), "shape")
